...
interface Props {
  listing: {
    ...
    listing_images: { url: string; is_primary: boolean; order: number }[];
    listing_tags: { name: string; value: string }[];
  };
}

export function BearbeitenForm({ listing }: Props) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const sortedImages = [...listing.listing_images].sort((a, b) => a.order - b.order);

  const [title, setTitle] = useState(listing.title);
  const [description, setDescription] = useState(listing.description);
  const [city, setCity] = useState(listing.city);
  const [zip, setZip] = useState(listing.zip ?? "");
  const [priceFrom, setPriceFrom] = useState(listing.price_from ? String(listing.price_from) : "");
  const [priceTo, setPriceTo] = useState(listing.price_to ? String(listing.price_to) : "");
  const [tags, setTags] = useState<{ name: string; value: string }[]>(listing.listing_tags);
  const [images, setImages] = useState<string[]>(sortedImages.map((i) => i.url));
  ...
  function toggleTag(name: string, value: string) {
    ...
    if (exists) {
      ...
      setTags([...tags.filter((t) => t.name !== name), { name, value }]);
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-7">
        <h1 className="text-2xl font-black text-white">Inserat bearbeiten</h1>
        <p className="text-zinc-500 text-sm mt-1">Änderungen werden sofort übernommen</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            >
              {uploading ? (
                ...
                <><ImagePlus className="w-5 h-5" /> Bilder hinzufügen</>
              )}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.push("/anbieter/dashboard")}
